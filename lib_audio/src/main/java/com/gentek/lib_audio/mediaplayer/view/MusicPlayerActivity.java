...
import com.gentek.lib_audio.mediaplayer.core.CustomMediaPlayer;
...
import com.gentek.lib_audio.mediaplayer.utils.Utils;
import com.gentek.lib_common_ui.base.BaseActivity;
import com.gentek.lib_common_ui.utils.StatusBarUtil;
...
public class MusicPlayerActivity extends FragmentActivity {
     ...
     * data
     */
    private SongBean mSongBean; //当前正在播放歌曲
    ...
    private void initData() {
        mSongBean = AudioController.getInstance().getNowPlaying();
        mPlayMode = AudioController.getInstance().getPlayMode();
    }

    private void initView() {
        mBgView = findViewById(R.id.root_layout);
        ImageLoaderManager.getInstance().displayImageForViewGroup(mBgView, mSongBean.getPic_premium(), true);
        ...
        findViewById(R.id.share_view).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                shareMusic(mSongBean.getUrlLink(), mSongBean.getTitle());
            }
        });
        ...
        mInfoView.setText(mSongBean.getAlbum_title());
        mInfoView.requestFocus();
        mAuthorView = findViewById(R.id.author_view);
        mAuthorView.setText(mSongBean.getAuthor());
    ...
    private void showPlayView() {
        mPlayView.setText(R.string.play_icon);
//        mPlayView.setImageResource(R.mipmap.audio_aj6);
    }

    private void showPauseView() {
        mPlayView.setText(R.string.pause_icon);
//        mPlayView.setImageResource(R.mipmap.audio_aj7);
    ...
    private void changeFavouriteStatus(boolean anim) {
        if (GreenDaoHelper.selectFavourite(mSongBean) != null) {
